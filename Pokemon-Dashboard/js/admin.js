...
function viewPlayerCollection(userId) {
    getUserById(userId, function(user) {
        ...
        var langOptions = Object.keys(_langFlags).map(function(k){ return '<option value="'+k+'">'+_langFlags[k]+' '+_langNames[k]+'</option>'; }).join('');

        html+='<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px;">'+
            '<div style="font-family:\'Press Start 2P\',monospace;font-size:9px;color:#1a1a2e;">📖 POKÉDEX ('+pokedex.length+')</div>'+
            (pokedex.length>0 ?
                '<div style="display:flex;align-items:center;gap:6px;">'+
                    '<span style="font-size:11px;color:#888;white-space:nowrap;">Mudar todas:</span>'+
                    '<select id="bulkLangSelect" style="padding:5px 8px;border:2px solid #e0e0e0;border-radius:8px;font-size:12px;font-family:inherit;">'+
                        langOptions+
                    '</select>'+
                    '<button class="btn-sm blue" onclick="changeAllLanguages(\''+userId+'\')" style="font-size:11px;white-space:nowrap;">✅ Aplicar todas</button>'+
                '</div>'
            : '')+
        '</div>';
        ...
        if(pokedex.length===0){
            ...
            pokedex.forEach(function(card){
                ...
                var langSelectInline = '<select onchange="changeCardLanguage(\''+userId+'\',\''+card.id+'\',this.value)" '+
                    'style="width:100%;margin-top:4px;padding:4px 6px;border:2px solid #e0e0e0;border-radius:6px;font-size:11px;font-family:inherit;">'+
                    Object.keys(_langFlags).map(function(k){
                        return '<option value="'+k+'"'+(k===lCode?' selected':'')+'>'+_langFlags[k]+' '+_langNames[k]+'</option>';
                    }).join('')+
                    '</select>';
                html+='<div class="collection-card">'+
                    ...
                    '<button onclick="toggleLeilaoCard(\''+userId+'\',\''+card.id+'\','+!!card.leilao+')" style="width:100%;margin-top:4px;padding:4px 6px;border-radius:6px;font-size:10px;font-weight:700;cursor:pointer;border:1px solid '+(card.leilao?'#f39c12':'#ddd')+';background:'+(card.leilao?'#fff8e6':'#f9f9f9')+';color:'+(card.leilao?'#f39c12':'#aaa')+';">'+
                        (card.leilao ? '🏷️ Leilão' : '🏷️ Marcar Leilão')+
                    '</button>'+
                    '<button class="btn-sm danger" onclick="removeFromPokedex(\''+userId+'\',\''+card.id+'\')" style="margin-top:4px;width:100%;font-size:11px;">🗑️ Remover</button>'+
                '</div>';
            });
            html+='</div>';
        }
...
function changeCardLanguage(userId, cardId, newLang) {
    getUserById(userId, function(user) {
        if(!user) return;
        var pokedex = user.pokedex||[];
        var card = pokedex.find(function(c){return c.id===cardId;});
        if(!card) return;
        card.language = newLang;
        updateUser(userId, {pokedex:pokedex}, function() {
            var langNames={'en':'Inglês','pt':'Português','ja':'Japonês','es':'Espanhol','fr':'Francês','de':'Alemão','it':'Italiano','ko':'Coreano'};
            showNotification('✅ Idioma de "'+card.name+'" → '+(langNames[newLang]||newLang),'success');
        });
    });
}

function changeAllLanguages(userId) {
    var sel = document.getElementById('bulkLangSelect');
    if(!sel) return;
    var newLang = sel.value;
    var langNames={'en':'Inglês','pt':'Português','ja':'Japonês','es':'Espanhol','fr':'Francês','de':'Alemão','it':'Italiano','ko':'Coreano'};
    if(!confirm('Mudar idioma de TODAS as cartas para '+( langNames[newLang]||newLang)+'?')) return;
    getUserById(userId, function(user) {
        if(!user) return;
        var pokedex = (user.pokedex||[]).map(function(c){ return Object.assign({},c,{language:newLang}); });
        updateUser(userId, {pokedex:pokedex}, function() {
            viewPlayerCollection(userId);
            showNotification('✅ Todas as cartas → '+(langNames[newLang]||newLang)+'!','success');
        });
    });
}

function toggleLeilaoCard(userId, cardId, isLeilao) {
    getUserById(userId, function(user) {
        if(!user) return;
        var pokedex = user.pokedex||[];
        var card = pokedex.find(function(c){ return c.id===cardId; });
        if(!card) return;
        card.leilao = !isLeilao;
        updateUser(userId, {pokedex:pokedex}, function() {
            viewPlayerCollection(userId);
            showNotification(card.leilao ? '🏷️ Marcada como Leilão!' : '✅ Marca de leilão removida!', 'success');
        });
    });
}
