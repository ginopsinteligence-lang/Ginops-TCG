...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var ss = document.getElementById('setSearchLeilao');
    if(ss) ss.addEventListener('keydown', function(e){ if(e.key==='Enter') searchCardsLeilao(); });
});
...
// ===== BUSCA POR SET =====
async function fetchAllCardsFromSet(setId) {
    var allCards = [];
    var page = 1;
    while(true) {
        var res  = await fetch('https://api.pokemontcg.io/v2/cards?q=set.id:'+encodeURIComponent(setId)+'&pageSize=250&page='+page+'&orderBy=number');
        var data = await res.json();
        var batch = data.data||[];
        allCards = allCards.concat(batch);
        if(allCards.length >= (data.totalCount||0) || batch.length === 0) break;
        page++;
    }
    return allCards;
}

async function searchCardsFromSet(setId) {
    var results = document.getElementById('cardResultsLeilao');
    results.innerHTML='<div class="loading-state">⏳ Carregando cartas do set...</div>';
    try {
        var cards = await fetchAllCardsFromSet(setId);
        if(cards.length===0){
            results.innerHTML='<div class="empty-state-main"><p>😢 Nenhuma carta encontrada</p></div>';
            return;
        }
        ...
        results.innerHTML='<div class="empty-state-main"><p>❌ Erro ao carregar cartas</p></div>';
    }
}
...
function setSearchModeLeilao(mode) {
    ...
    var bySet    = document.getElementById('searchBySet');
    var byManual = document.getElementById('searchByManual');

    byName.style.display   = 'none';
    byNumber.style.display = 'none';
    if (bySet)    bySet.style.display    = 'none';
    if (byManual) byManual.style.display = 'none';

    // Reset botões
    ['modeByName','modeByNumber','modeBySet','modeByManual'].forEach(function(id){
    ...
    if (mode === 'name') {
        ...
        if (bySet) bySet.style.display = 'flex';
        document.getElementById('modeBySet').classList.add('active');
    } else if (mode === 'manual') {
...
async function searchCardsLeilao() {
    ...
    try {
        ...
        if(_searchMode==='set') {
            var setInput = document.getElementById('setSearchLeilao').value.trim();
            if(!setInput){ results.innerHTML=''; return; }
            // Busca sets cujo total bate com o número digitado
            var setsRes  = await fetch('https://api.pokemontcg.io/v2/sets?q=total:'+encodeURIComponent(setInput)+'&pageSize=20');
            var setsData = await setsRes.json();
            var sets     = setsData.data||[];
            if(sets.length === 0){
                results.innerHTML='<div class="empty-state-main"><p>😢 Nenhum set encontrado com '+setInput+' cartas</p></div>';
                return;
            }
            // Se houver mais de um set, mostra opções para o usuário escolher
            if(sets.length > 1) {
                results.innerHTML =
                    '<div style="padding:10px 0;font-size:12px;color:var(--text2);margin-bottom:8px;">'+sets.length+' sets encontrados com '+setInput+' cartas. Escolha um:</div>'+
                    '<div style="display:flex;flex-direction:column;gap:6px;max-height:220px;overflow-y:auto;">'+
                    sets.map(function(s){
                        return '<div onclick="searchCardsFromSet(\''+s.id+'\')" style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--bg2);border:1px solid var(--border);border-radius:10px;cursor:pointer;transition:border-color .2s;" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'+
                            (s.images&&s.images.symbol?'<img src="'+s.images.symbol+'" style="width:24px;height:24px;object-fit:contain;">':'')+
                            '<div><div style="font-size:13px;font-weight:700;">'+escapeHtml(s.name)+'</div>'+
                            '<div style="font-size:11px;color:var(--text2);">'+s.total+' cartas · '+s.releaseDate+'</div></div>'+
                        '</div>';
                    }).join('')+
                    '</div>';
                return;
            }
            // Só um set — busca direto as cartas
            cards = await fetchAllCardsFromSet(sets[0].id);

        } else if(_searchMode==='number') {
            ...
            // Busca as duas variações em paralelo (com e sem zero)
            var urls = [
                'https://api.pokemontcg.io/v2/cards?q=number:'+encodeURIComponent(numInt)+'&pageSize=60&orderBy=name',
                'https://api.pokemontcg.io/v2/cards?q=number:'+encodeURIComponent(numPart)+'&pageSize=60&orderBy=name'
            ];
            var responses = await Promise.all(urls.map(function(u){ return fetch(u).then(function(r){ return r.json(); }); }));
            // Junta e remove duplicatas pelo id
            var seen = {};
            cards = [];
            responses.forEach(function(d){
                (d.data||[]).forEach(function(c){
                    if(!seen[c.id]){ seen[c.id]=true; cards.push(c); }
                });
            });

            // Filtra pelo total do set se digitou "018/131"
            ...
            // Filtra pelo número (aceita com ou sem zero: "018" ou "18")
            cards = cards.filter(function(c){
                return c.number === numPart || c.number === numInt || parseInt(c.number,10) === parseInt(numPart,10);
            });
        } else {
