...
function setSearchModeLeilao(mode) {
    _searchMode = mode;
    document.getElementById('searchByName').style.display   = mode==='name'   ? 'flex'  : 'none';
    document.getElementById('searchByNumber').style.display = mode==='number' ? 'flex'  : 'none';
    document.getElementById('searchManual').style.display   = mode==='manual' ? 'block' : 'none';

    document.getElementById('modeByName').classList.toggle('active',   mode==='name');
    document.getElementById('modeByNumber').classList.toggle('active', mode==='number');
...
// ===== CLOUDINARY UPLOAD =====
var CLOUDINARY_CLOUD = 'db3wvvv2l';
var CLOUDINARY_PRESET = 'GinopsTCG';

async function uploadImagemCloudinary(input) {
    var file = input.files[0];
    if(!file) return;

    var status = document.getElementById('manualImagemStatus');
    var preview = document.getElementById('manualImagemPreview');
    var previewImg = document.getElementById('manualImagemPreviewImg');
    ...
    try {
        ...
        if(data.secure_url) {
            document.getElementById('manualImagem').value = data.secure_url;
...
function addItemManual() {
    var nome   = document.getElementById('manualNome').value.trim();
    var set    = document.getElementById('manualSet').value.trim();
    var numero = document.getElementById('manualNumero').value.trim();
    var rarity = document.getElementById('manualRaridade').value.trim();
    var imagem = document.getElementById('manualImagem').value.trim();
    var valor  = parseFloat(document.getElementById('manualValor').value);

    if(!nome){ showNotification('⚠️ Digite o nome da carta!','error'); return; }
    ...
    saveLeilao(_currentLeilao.id, {itens:itens}, function(){
        ...
        document.getElementById('manualRaridade').value = '';
        document.getElementById('manualImagem').value   = '';
        document.getElementById('manualValor').value    = '';
        document.getElementById('manualImagemPreview').style.display = 'none';
        document.getElementById('manualImagemFile').value = '';
        showNotification('✅ "'+nome+'" adicionado!','success');
    });
}
