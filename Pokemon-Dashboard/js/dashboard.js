...
function loadPokedex(user) {
    ...
    grid.innerHTML=cards.map(function(card){
        var langFlags={'en':'🇺🇸','pt':'🇧🇷','ja':'🇯🇵','es':'🇪🇸','fr':'🇫🇷','de':'🇩🇪','it':'🇮🇹','ko':'🇰🇷'};
        var langFlag = langFlags[card.language||'en'] || '🌍';
        return '<div class="card-item" onclick="showCardDetails(\''+card.id+'\')">'+
            '<img src="'+card.image+'" alt="'+escapeHtml(card.name)+'" loading="lazy">'+
            (card.leilao ? '<div style="background:#f39c12;color:white;font-size:9px;font-weight:800;padding:2px 6px;border-radius:99px;display:inline-block;margin-bottom:3px;">🏷️ LEILÃO</div>' : '')+
            '<div class="card-name">'+escapeHtml(card.name)+'</div>'+
            '<span class="card-rarity '+rarityClass(card.rarity)+'">'+card.rarity+'</span>'+
            '<div style="font-size:11px;color:#888;margin-top:2px;">'+langFlag+' '+formatDate(card.addedAt)+'</div>'+
        '</div>';
    }).join('');
}
...
function showCardDetails(cardId) {
    ...
    getUserById(cu.id, function(user) {
        ...
        var langCode = card.language || 'en';
        var langLabel = (langFlags[langCode]||'🌍') + ' ' + (langNames[langCode]||langCode);
        document.getElementById('cardModal').querySelector('.modal-card-content').innerHTML=
            ...
            '<div class="modal-card-details">'+
                ...
                '<div class="detail-row"><span class="detail-label">🌍 Idioma</span><span class="detail-value">'+langLabel+'</span></div>'+
                (card.number ? '<div class="detail-row"><span class="detail-label">🔢 Número</span><span class="detail-value">'+card.number+'</span></div>' : '')+
                '<div class="detail-row"><span class="detail-label">📅 Capturado</span><span class="detail-value">'+formatDate(card.addedAt)+'</span></div>'+
                (card.leilao ? '<div class="detail-row"><span class="detail-label">🏷️ Origem</span><span class="detail-value" style="color:#f39c12;font-weight:800;">Leilão</span></div>' : '')+
                (card.manual ? '<div class="detail-row"><span class="detail-label">📷 Tipo</span><span class="detail-value">Cadastro Manual</span></div>' : '')+
            '</div></div>';
